...
public class OpponentEngineMod : MonoBehaviour
{
    ...
    public bool brakeToAvoidObstacle = false;
    public bool doNotReverse = false;
    [SerializeField] private bool testingMode = false;

    public enum AllStates { Idle, FollowPath, AvoidObstacles, ControlSpeed, DeadlockAvoidance };
    public AllStates opponentState;
    ...
    private void Update()
    {
        switch (opponentState)
        {
            case AllStates.Idle:
                ...
                if (path != null)
                {
                    opponentState = AllStates.FollowPath;
                }
            ...
            case AllStates.AvoidObstacles:
                MoveCar(baseSpeed / 2);
                SteerCar(); // Increase steering range.
                SenseObstacles();
                LerpToAvoidObstacles();
                break;

            case AllStates.DeadlockAvoidance:
                // Reverse + Negative steer? + Respawn logic
                break;

            // Else give up all hope.
            default:
                break;
        }
    }
    ...
    private void SenseObstacles()
    {
        ...
        if (Physics.Raycast(sensorPosition, transform.forward, out hit, sensingDistance))
        {
            if (!hit.collider.CompareTag("Terrain")) //  || Terrain.activeTerrain.SampleHeight(hit.point) > 0.6f)
            {
                ...
                if (hit.collider.CompareTag("Opponent") || hit.collider.CompareTag("Player"))
                {
                    ...
                    if (vehicleSpeed > 15 && distanceFromOpponent < 5)
                    {
                        brakeToAvoidObstacle = true;
                    }
                    else { brakeToAvoidObstacle = false; }
                    avoidingSensitivity = Mathf.Lerp(0.01f, 0.2f, Mathf.InverseLerp(3f, 0.5f, distanceFromOpponent));
                ...
                else
                {
                    ...
                    vehicleSpeed = vehicleRB.velocity.magnitude;
                    if (vehicleSpeed > 15 && distanceFromObstacle < 5)
                    {
                        brakeToAvoidObstacle = true;
                    }
                    else { brakeToAvoidObstacle = false; }
                    avoidingSensitivity = Mathf.Lerp(0.05f, 0.7f, Mathf.InverseLerp(25f, 1f, distanceFromObstacle));
        ...
        else if (Physics.Raycast(sensorPosition, Quaternion.AngleAxis(-frontSensorAngle, transform.up) * transform.forward, out hit, sensingDistance))
        {
            if (!hit.collider.CompareTag("Terrain")) //  || Terrain.activeTerrain.SampleHeight(hit.point) > 0.6f)
            {
                ...
                if (hit.collider.CompareTag("Opponent") || hit.collider.CompareTag("Player"))
                {
                    ...
                    avoidingSensitivity = Mathf.Lerp(0.05f, 0.1f, Mathf.InverseLerp(3f, 0.5f, distanceFromOpponent));
                    // if (distanceFromOpponent < 1f)
                    // {
                    //     avoidingSensitivity += 0.1f;
                    // }
                ...
                else
                {
                    ...
                    avoidingSensitivity = Mathf.Lerp(0.05f, 0.3f, Mathf.InverseLerp(15f, 1f, distanceFromObstacle));
                    // if (distanceFromObstacle < 3f)
                    // {
                    //     // print("Too close!");
                    //     avoidingSensitivity += 0.3f;
        ...
        if (avoidingSensitivity == 0)
        {
            if (Physics.Raycast(sensorPosition, transform.forward, out hit, sensingDistance))
            {
                if (!hit.collider.CompareTag("Terrain")) //  || Terrain.activeTerrain.SampleHeight(hit.point) > 0.6f)
                {
                    Debug.DrawLine(sensorPosition, hit.point, Color.yellow);
                    currentlyAvoiding = true;
                }
        ...
        {
            targetSteerAngle = maxSteerAngle * avoidingSensitivity * 1f;
        }
    }
    ...
    private void ApplyBrakes()
    {
        ...
        if (isBraking || forceApplybrakes || brakeToAvoidObstacle)
        {
